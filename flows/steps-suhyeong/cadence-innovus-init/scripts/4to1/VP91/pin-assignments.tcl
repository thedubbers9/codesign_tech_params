...
set block_width [expr $base_pitch * 18 + $xoffset * 2]
...
# fake in & out
set pitchY [expr $base_pitch * 6]
set pitchX [expr $base_pitch * 5]
set odd 1
set y $yoffset
set x $xoffset
...
foreach port $real_in_ports {
  set x [expr [expr {int($idx / [expr int($edge)])}] * $pitchX + $xoffset * 3 + $block_width]
